...
Tweaks can declare MCP servers in `manifest.json`. CodexDC syncs enabled
...
CodexDC writes managed entries between markers:
...
If a generated name conflicts with another managed name, CodexDC appends
`-2`, `-3`, and so on.

If the user already has a manual `[mcp_servers.<name>]` entry outside the
managed block, CodexDC skips that managed server instead of overwriting it.
