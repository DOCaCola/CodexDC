...
If a tweak declares `githubRepo` in `owner/repo` form, CodexDC checks GitHub
Releases at most once per day per installed tweak:
...
CodexDC does not auto-install arbitrary tweak releases.
...
When updating a store tweak, CodexDC checks whether local files differ from the
...
```sh
codexdc create-tweak ./my-tweak --id com.you.my-tweak --name "My Tweak" --repo you/my-tweak
codexdc validate-tweak ./my-tweak
codexdc dev ./my-tweak
codexdc status
codexdc doctor
codexdc debug
```
...
Renderer logs also appear in DevTools with `[codexdc]` prefixes.

## Debugging Renderer Tweaks

- Filter DevTools console for `[codexdc]`.
