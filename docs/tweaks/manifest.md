...
| `githubRepo` | `owner/repo` | Optional. Enables advisory GitHub release checks. Must match `owner/repo` when present. |
...
| `mcp` | `TweakMcpServer` | Declares an MCP server managed by CodexDC. |
...
the permission is declared. CodexDC resolves the real target path and rejects
...
```sh
codexdc validate-tweak ./my-tweak
```
...
```js
const { validateTweakManifest } = require("@codexdc/sdk");
...
CodexDC checks GitHub Releases at most once per day per installed tweak:
...
If `githubRepo` is omitted, the tweak still loads normally but CodexDC skips
release checks and repo links for that tweak.

CodexDC does not auto-install arbitrary tweak releases. Store-approved tweaks
