...
CodexDC 1.0.0 exposes native macOS integration through `api.codex.native`.
...
CodexDC checks real paths, so symlink escapes are rejected.

## Built-In Host

When `moduleId` is omitted, CodexDC uses its bundled Objective-C++ native host:
...
3. Export the CodexDC native module contract from the shim.
...
CodexDC tracks native modules, panels, views, and helpers per tweak.
