...
This is the full public API surface exported by `@codexdc/sdk`.
...
```ts
import { defineTweak } from "@codexdc/sdk";
...
It does not check that the entry file exists; `codexdc validate-tweak`
...
Declares an MCP server managed by CodexDC. See [MCP servers](./mcp.md).
...
When `route` is provided, CodexDC loads the Codex app route through
...
CodexDC prefers Owl's private `contentView.addChildView(view.webContentsView)`
...
CDP is opt-in. CodexDC reports status but does not silently enable it.
...
`path` is resolved inside the tweak directory. The file must exist, and CodexDC
...
For `.node` modules, CodexDC calls `module.request(method, payload)` when
...
CodexDC uses its built-in native host and defaults `factory` to `createPanel`.
The runtime supplies the parent native window handle to the module when
available. After creation,
CodexDC forwards parent window bounds, focus, visibility, fullscreen, minimize,
...
When `moduleId` is omitted, CodexDC uses its built-in native host and defaults
...
Native APIs are main-process backed. Renderer tweaks call them through CodexDC
