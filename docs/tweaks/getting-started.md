...
`codexdc dev` validates the manifest, creates a symlink, touches a reload
marker, and watches for local changes. Pass `--replace` if a symlink with that
name already points somewhere else.

## Entry File Resolution

CodexDC resolves the entry file in this order:
  ...
  start(api) {
    api.settings.registerPage({
      ...
      render(root) {
        ...
        p.textContent = "Hello from CodexDC.";
        root.append(p);
      },
    });
  },
