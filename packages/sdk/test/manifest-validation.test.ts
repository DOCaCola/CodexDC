...
test("validateTweakManifest accepts a manifest without githubRepo", () => {
  const result = validateTweakManifest({
    id: "com.example.local-tweak",
    name: "Local Tweak",
    version: "0.1.0",
    scope: "renderer",
  });
...
test("validateTweakManifest rejects invalid githubRepo when provided", () => {
  const result = validateTweakManifest({
    id: "com.example.tweak",
    name: "Example Tweak",
    version: "0.1.0",
  ...
  assert.equal(result.ok, false);
  assert.deepEqual(result.errors.map((issue) => issue.path), ["githubRepo"]);
});
