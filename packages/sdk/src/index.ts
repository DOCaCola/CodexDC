/**
 * Public SDK for codexdc tweak authors.
 *
...
export interface TweakManifest {
  ...
  version: string;
  /**
   * Optional source repository in `owner/repo` form. When present, CodexDC
   * checks this repository's latest GitHub release once per day and only
   * reports update availability; it never downloads or installs tweak updates
   * automatically.
   */
  githubRepo?: string;
  ...
  main?: string;
  /**
   * Optional MCP server exposed by this tweak. CodexDC syncs this into Codex's
...
export function validateTweakManifest(manifest: unknown): TweakManifestValidationResult {
  ...
  if (
    manifest.githubRepo !== undefined &&
    (typeof manifest.githubRepo !== "string" ||
      !/^[a-zA-Z0-9._-]+\/[a-zA-Z0-9._-]+$/.test(manifest.githubRepo))
  ) {
    errors.push({
      path: "githubRepo",
      message: "githubRepo must use owner/repo format when provided",
    });
  }
  ...
  if (manifest.permissions !== undefined) {
    if (
      ...
      errors.push({
        path: "permissions",
        message: "permissions must be known CodexDC permission strings",
      });
    }
  }
...
export interface TweakApi {
  ...
  /** Per-tweak logger; output goes to the codexdc log file + devtools. */
  log: TweakLogger;
  ...
  /** Renderer-only: register commands in Codex's composer UI. */
  ...
  fs: TweakFs;
  /** Main-only: native Codex integration points exposed by CodexDC. */
  codex?: CodexApi;
}
...
export interface SettingsApi {
  /**
   * Register a section in the CodexDC "Tweaks" page. The section appears as
...
export interface ComposerApi {
  slashCommands: SlashCommandsApi;
}

export interface SlashCommandsApi {
  /**
   * Register a renderer-owned slash command. Exact command submission is
   * consumed before it reaches Codex's CLI.
   */
  register(command: SlashCommand): SlashCommandHandle;
}

export interface SlashCommand {
  /** Command text without the leading slash, for example `restart-session`. */
  name: string;
  /** Human-readable action title. */
  title: string;
  /** Optional explanation shown alongside the command. */
  description?: string;
  /** Optional command names without the leading slash. */
  aliases?: string[];
...
export interface CodexApi {
  ...
  /** Permission-gated access to local files outside the tweak data directory. */
  files: CodexFilesApi;
...
export interface CodexAppServerRestartOptions {
  /** Host to restart. Defaults to the local Codex backend. */
  hostId?: string;
  /** Request termination of the managed Codex process before reconnecting. */
  killCodexProcess?: boolean;
}
...
export interface CodexRuntimeApi {
  ...
  /** Restart the desktop-owned Codex app-server and reconnect the renderer. */
  restartAppServer(options?: CodexAppServerRestartOptions): Promise<void>;
}
...
export interface CodexFilesApi {
