...
test("Windows desktop run check ignores CLI executables with the same basename", () => {
  ...
  assert.equal(
    matchesCodexMainExecutable(codex, "D:\\Projekte\\Codex Cli Fork\\output\\codex.exe"),
    false,
  );
  assert.equal(matchesCodexMainExecutable(codex, `${codex.executable} --type=renderer`), true);
});
