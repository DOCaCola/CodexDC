...
test("self-update only downloads from an explicitly configured repository", () => {
  assert.equal(resolveSelfUpdateRepo("example/option", "example/env", "example/config"), "example/option");
  assert.equal(resolveSelfUpdateRepo(undefined, "example/env", "example/config"), "example/env");
  assert.equal(resolveSelfUpdateRepo(undefined, undefined, "example/config"), "example/config");
  assert.equal(resolveSelfUpdateRepo("  ", "", "   "), null);
  assert.equal(resolveSelfUpdateRepo(), null);
});
...
test("cli documents local signing and safe mode recovery", () => {
  ...
  assert.match(source, /Leave safe mode with: codexdc safe-mode --off/);
...
test("installation source labels local checkouts", () => {
  ...
  assert.equal(
    describeInstallationSource("/opt/homebrew/Cellar/codexdc/0.1.4").kind,
    "homebrew",
  );
});
...
async function withTempEnvAsync(fn: (root: string) => Promise<void>): Promise<void> {
  ...
  const originalHome = process.env.CODEXDC_HOME;
  process.env.CODEXDC_HOME = root;
  try {
    await fn(root);
  } finally {
    if (originalHome === undefined) delete process.env.CODEXDC_HOME;
    else process.env.CODEXDC_HOME = originalHome;
    rmSync(root, { recursive: true, force: true });
  }
}
