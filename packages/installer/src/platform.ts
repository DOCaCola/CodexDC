...
export interface CodexInstall {
  ...
  appName: string;
  /** Windows AppUserModelId, when declared by the package manifest. */
...
function locateMac(override?: string): CodexInstall {
  ...
  const appRoot = unique(candidates).find((p) => isMacCodexApp(p) && (Boolean(override) || readMacAppInfo(p).bundleId !== "io.github.docacola.codexdc"));
  if (!appRoot) {
    throw new Error(
      `[!] Codex App Not Found\n\n` +
...
export function inferCodexChannel(bundleId: string | null, appName?: string): CodexChannel {
  if (bundleId === "io.github.docacola.codexdc") return "stable";
  ...
  if (/\b(codex|chatgpt)\b/i.test(appName ?? "")) return "stable";
  return "unknown";
}
...
function locateWin(override?: string): CodexInstall {
  ...
  if (!appRoot) {
    ...
    if (storeInstalls.length > 0) {
      ...
      throw new Error(
        `[!] Codex App Not Found\n\n` +
          `Codex appears to be installed from the Microsoft Store, but CodexDC could not find app.asar under the expected package layout.\n\n` +
          `Store package(s):\n${storeText}\n\n` +
          `Expected one of:\n` +
  ...
  const declaredExecutable = readWindowsAppExecutableName(appRoot);
  const executable = resolveWindowsExecutable(writableAppRoot, declaredExecutable);
  const appName = basename(executable, ".exe");
  const appUserModelId = readWindowsAppUserModelId(writableAppRoot);
  return {
    appRoot: writableAppRoot,
    sourceAppRoot: writableAppRoot !== appRoot ? appRoot : undefined,
    resourcesDir,
    asarPath: join(resourcesDir, "app.asar"),
    metaPath: null,
...
export function hasUsableWindowsStoreMirror(appRoot: string): boolean {
  return isWinCodexRoot(appRoot);
}
...
export function readWindowsAppExecutableName(appRoot: string): string | null {
  const manifest = readWindowsAppManifest(appRoot);
  const match = manifest
    ? /<Application\b[^>]*\bExecutable\s*=\s*["']([^"']+)["']/i.exec(manifest)
    : null;
  return match?.[1] ? basename(match[1].replace(/\\/g, "/")) : null;
}

export function readWindowsAppUserModelId(appRoot: string): string | null {
  const manifestPath = findWindowsAppManifest(appRoot);
  const manifest = manifestPath ? readWindowsAppManifest(appRoot) : null;
  if (manifestPath && manifest) {
    const identityName = xmlAttribute(manifest, "Identity", "Name");
    const applicationId = xmlAttribute(manifest, "Application", "Id");
    const publisherId = publisherIdFromPackageRoot(dirname(manifestPath));
    if (identityName && applicationId && publisherId) {
      return `${identityName}_${publisherId}!${applicationId}`;
    }
  }

  return inferWindowsAppUserModelIdFromPath(appRoot);
}

function findWindowsAppManifest(appRoot: string): string | null {
  const manifestPaths = [
    join(dirname(appRoot), "AppxManifest.xml"),
    join(appRoot, "AppxManifest.xml"),
  ];
  return manifestPaths.find((manifestPath) => existsSync(manifestPath)) ?? null;
}

function readWindowsAppManifest(appRoot: string): string | null {
  const manifestPath = findWindowsAppManifest(appRoot);
...
function publisherIdFromPackageRoot(packageRoot: string): string | null {
  return /__([^_\\/]+)$/i.exec(basename(packageRoot))?.[1] ?? null;
}
...
function inferWindowsAppUserModelIdFromPath(appRoot: string): string | null {
  ...
  const packageName = basename(dirname(appRoot));
  const match = /^(.+?)_\d+(?:\.\d+)+_[^_]+__([^_]+)$/i.exec(packageName);
  if (!match) return null;
  return `${match[1]}_${match[2]}!App`;
}

export function resolveWindowsExecutable(appRoot: string, preferredName?: string | null): string {
  const names = [
    ...new Set(
      [preferredName?.trim(), "ChatGPT.exe", "Codex.exe"].filter(
