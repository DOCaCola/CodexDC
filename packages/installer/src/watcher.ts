 ...
 *   macOS:   ~/Library/LaunchAgents/com.codexdc.watcher.plist (launchd)
 *   Linux:   ~/.config/systemd/user/codexdc-watcher.service (systemd --user)
 *   Windows: Task Scheduler entry via schtasks.exe
 *
 * The watcher itself is just `codexdc repair --quiet` triggered on the
...
const LABEL = "com.codexdc.watcher";
...
function launchdLogPath(): string {
  return join(targetUserHome(), "Library", "Logs", "codexdc-watcher.log");
}
...
function isRunningFromWatcher(): boolean {
  return process.env.CODEXDC_WATCHER === "1" || process.env.XPC_SERVICE_NAME === LABEL;
}
...
function installSystemd(appRoot: string): WatcherKind {
  ...
  const unit = `[Unit]
Description=codexdc repair watcher
...
`;
  writeFileSync(join(dir, "codexdc-watcher.service"), unit);
  writeFileSync(join(dir, "codexdc-watcher.timer"), `[Unit]
Description=codexdc repair watcher interval
...
`);
  writeFileSync(join(dir, "codexdc-watcher.path"), `[Unit]
Description=codexdc app.asar watcher
...
`);
  try {
    ...
    execFileSync("systemctl", ["--user", "enable", "codexdc-watcher.service"], {
      stdio: "ignore",
    });
    execFileSync("systemctl", ["--user", "enable", "--now", "codexdc-watcher.timer"], {
      stdio: "ignore",
    });
    execFileSync("systemctl", ["--user", "enable", "--now", "codexdc-watcher.path"], {
      stdio: "ignore",
    });
  } catch {
...
function uninstallSystemd(): void {
  const path = join(homedir(), ".config", "systemd", "user", "codexdc-watcher.service");
  if (!existsSync(path)) return;
  try {
    execFileSync("systemctl", ["--user", "disable", "codexdc-watcher.service"], {
      stdio: "ignore",
    });
    execFileSync("systemctl", ["--user", "disable", "--now", "codexdc-watcher.path"], {
      stdio: "ignore",
    });
    execFileSync("systemctl", ["--user", "disable", "--now", "codexdc-watcher.timer"], {
      stdio: "ignore",
    });
  } catch {}
  rmSync(path, { force: true });
  rmSync(join(homedir(), ".config", "systemd", "user", "codexdc-watcher.path"), {
    force: true,
  });
  rmSync(join(homedir(), ".config", "systemd", "user", "codexdc-watcher.timer"), {
    force: true,
  });
}
...
function installScheduledTask(_appRoot: string): WatcherKind {
  ...
  try {
    deleteScheduledTask("codexdc-watcher-daily");
    ...
    deleteScheduledTask("codexdc-watcher-hourly");
    deleteScheduledTask("codexdc-watcher-interval");
    execFileSync("schtasks.exe", [
      "/Create",
      "/F",
      "/SC",
...
function windowsCodexPlusPlusDir(): string {
  return join(process.env.APPDATA ?? join(homedir(), "AppData", "Roaming"), "codexdc");
}

function uninstallScheduledTask(): void {
  deleteScheduledTask("codexdc-watcher");
  deleteScheduledTask("codexdc-watcher-interval");
  deleteScheduledTask("codexdc-watcher-hourly");
  deleteScheduledTask("codexdc-watcher-daily");
}
