...
export async function uninstall(opts: Opts = {}): Promise<void> {
  ...
  if (isCodexRunning(codex.appRoot)) {
    throw new Error(
      `[!] Close Codex before uninstalling CodexDC\n\n` +
  ...
  if (opts.purge) {
    purgeUserData(paths);
    console.log(kleur.green("Removed CodexDC user data."));
  } else {
    console.log(
...
export function chooseRestorePlan(input: {
  ...
  if (!appLooksPatched) {
    ...
    if (input.currentAsarHash === null) {
      return { kind: "skip", reason: "current app.asar could not be inspected and no CodexDC marker was found" };
    }
    return {
      kind: "skip",
      reason: "current app does not appear to contain the CodexDC patch",
    };
  }
  ...
  if (
    ...
    throw new Error(
      `Cannot safely uninstall with partial backups because Codex changed since CodexDC was installed.\n\n` +
        `Installed against: ${input.state.codexVersion}\n` +
        `Current Codex:     ${input.currentCodexVersion}\n\n` +
        `Update or reinstall Codex from the official app, then remove CodexDC state manually if needed.`,
    );
  }
...
function restorePartialBackup(
  ...
  if (codex.platform === "win32") {
    ...
    setIntegrity(codex, restoredHash);
  }
...
function cleanupPermissionError(error: unknown, path: string, label: string): Error {
  ...
  return new Error(
    `Cannot remove CodexDC ${label} at ${path}.\n` +
