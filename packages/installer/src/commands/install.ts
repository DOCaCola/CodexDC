...
import {
  ...
  type WindowsShellHarnessSourceDiagnostics,
} from "../windows-shell-harness.js";
...
export async function install(opts: Opts = {}): Promise<void> {
  ...
  // 4. Update the host's embedded integrity metadata so the patched archive
  // remains launchable. Current Windows Owl builds store this inside the PE.
  const integrityUpdate = integrityPlan?.write(patchedAsarHash) ?? null;
  if (integrityUpdate) {
    step.detail(
      `Updated ${integrityUpdate.location === "windows-pe" ? "Windows embedded ASAR integrity" : "ElectronAsarIntegrity"} → ` +
        `${kleur.dim(patchedAsarHash.slice(0, 12))}…`,
    );
  }
  ...
  writeState(paths.stateFile, {
    ...
    managedCopy: true,
    officialAppRoot: codex.sourceAppRoot,
  });
  chownForTargetUser(paths.root, { recursive: true });
  const launcher = installWindowsManagedAppLauncher(codex);
  if (launcher) {
    step(
      `Installed patched CodexDC launcher${launcher.shortcutPaths.length === 1 ? "" : "s"}: ${launcher.shortcutPaths.map((p) => kleur.cyan(p)).join(", ")}`,
    );
  }
  const mirrorRetention = pruneWindowsStoreMirrors(codex.appRoot);
  if (mirrorRetention.removed.length > 0) {
    step(`Removed ${mirrorRetention.removed.length} old CodexDC Store mirror${mirrorRetention.removed.length === 1 ? "" : "s"}`);
  }
  if (mirrorRetention.failed.length > 0) {
    console.warn(
      kleur.yellow(
        `Could not remove ${mirrorRetention.failed.length} old CodexDC Store mirror${mirrorRetention.failed.length === 1 ? "" : "s"}; cleanup will retry on the next install or repair.`,
      ),
    );
  }
  if (reopenAfterPatch) {
  ...
  if (!opts.quiet) {
    console.log();
    console.log(kleur.green().bold("✓ codexdc installed."));
    console.log(`  Tweaks: ${kleur.cyan(paths.tweaks)}`);
    console.log(`  Logs:   ${kleur.cyan(paths.logDir)}`);
    if (launcher) {
      console.log(`  Launch ${kleur.cyan("CodexDC")} from Start Menu or Desktop.`);
...
export function hasCodexPlusPlusAsarMarker(asarPath: string): boolean {
  try {
    ...
    return pkg.main === "codexdc-loader.cjs" || typeof pkg.__codexpp === "object";
  } catch {
    return false;
  }
}
...
async function injectLoader(
  asarPath: string,
  userRoot: string,
  appUserModelId: string | null | undefined,
  ...
  await patchAsar(asarPath, (dir) => {
    ...
    const existingMeta =
      pkg["__codexpp"] && typeof pkg["__codexpp"] === "object"
        ? (pkg["__codexpp"] as Record<string, unknown>)
        : null;

    // Already patched? Refresh metadata that the loader needs.
    if (existingMeta) {
      originalMain = String(existingMeta.originalMain ?? "");
      if (appUserModelId && existingMeta.appUserModelId !== appUserModelId) {
        existingMeta.appUserModelId = appUserModelId;
      ...
      pkg["__codexpp"] = {
        originalMain,
        userRoot,
        loader: "codexdc-loader.cjs",
        ...(appUserModelId ? { appUserModelId } : {}),
      };
      pkg.main = "codexdc-loader.cjs";
      writeFileSync(pkgPath, JSON.stringify(pkg, null, 2));
    }
    ...
    if (!existsSync(loaderSrc)) {
      ...
      cpSync(devLoader, join(dir, "codexdc-loader.cjs"));
    } else {
      cpSync(loaderSrc, join(dir, "codexdc-loader.cjs"));
    }

    patchCodexWindowServices(dir, originalMain, step);
    patchWindowsShellHarness(dir, step);
  });
  return originalMain;
}
...
interface WindowsShellHarnessCandidateDiagnostic {
  relativePath: string;
  bytes: number;
  diagnostics: WindowsShellHarnessSourceDiagnostics;
  parserError?: string;
}
...
export function patchWindowsShellHarness(appDir: string, step: (msg: string) => void = () => {}): void {
  const candidates = findWindowsShellHarnessCandidates(appDir);
  const candidateNames = candidates.map((p) => relative(appDir, p) || basename(p));
  step(
    `Scanning Windows shell harness candidates (${candidates.length}): ${
      candidateNames.length ? candidateNames.map((p) => kleur.dim(p)).join(", ") : kleur.yellow("none")
    }`,
  );
  const diagnostics: WindowsShellHarnessCandidateDiagnostic[] = [];
  let patchedCandidates = 0;
  let guardCompatibleCandidates = 0;
  let envCompatibleCandidates = 0;

  for (const filePath of candidates) {
    const source = readFileSync(filePath, "utf8");
    const relativePath = relative(appDir, filePath) || basename(filePath);
    const candidateDiagnostic: WindowsShellHarnessCandidateDiagnostic = {
      relativePath,
      bytes: source.length,
      diagnostics: describeWindowsShellHarnessSource(source, WINDOWS_SHELL_HARNESS_KEY),
    };
    diagnostics.push(candidateDiagnostic);

    let patched: ReturnType<typeof patchWindowsShellHarnessSource> = null;
    try {
      patched = patchWindowsShellHarnessSource(source, WINDOWS_SHELL_HARNESS_KEY);
    } catch (e) {
      candidateDiagnostic.parserError = (e as Error).message;
      continue;
    }
    ...
    if (patched) {
      patchedCandidates += 1;
      if (patched.guardStatus !== "missing") guardCompatibleCandidates += 1;
      if (patched.envStatus !== "missing") envCompatibleCandidates += 1;
      if (patched.changed) writeFileSync(filePath, patched.source);
      const action =
        patched.strategy === "already-fixed"
          ? "Detected permissive Windows shell harness"
          : patched.strategy === "already-patched"
            ? "Detected existing Windows shell harness patch"
            : "Patched Windows shell harness";
      step(
        `${action} in ${kleur.dim(relativePath)} using ${kleur.cyan(patched.strategy)}${
          patched.matchCount > 1 ? ` (${patched.matchCount} matches)` : ""
        }`,
      );
    }
  }

  if (patchedCandidates > 0 && guardCompatibleCandidates > 0 && envCompatibleCandidates > 0) return;

  throw new Error(formatWindowsShellHarnessFailure(diagnostics));
}
...
function findWindowsShellHarnessCandidates(appDir: string): string[] {
  const buildDir = resolve(appDir, ".vite", "build");
  const roots: Array<{ dir: string; recursive: boolean }> = [
    { dir: buildDir, recursive: true },
    { dir: appDir, recursive: false },
  ...
  return roots
    .flatMap((root) => collectJavaScriptFiles(root.dir, root.recursive))
    .sort((a, b) => {
      const rank = windowsShellHarnessCandidateRank(basename(a)) - windowsShellHarnessCandidateRank(basename(b));
      if (rank !== 0) return rank;
      const name = basename(a).localeCompare(basename(b));
      if (name !== 0) return name;
      return relative(appDir, a).localeCompare(relative(appDir, b));
...
function windowsShellHarnessCandidateRank(name: string): number {
  if (/^src(?:[-.].*)?\.js$/.test(name)) return 0;
  if (/^main(?:[-.].*)?\.js$/.test(name)) return 1;
  if (/^(app|desktop)(?:[-.].*)?\.js$/.test(name)) return 2;
  if (/worker|preload|renderer/i.test(name)) return 4;
  return 3;
}
...
function formatWindowServicesHookFailure(
  ...
  const lines = [
    ...
    "",
    "CodexDC could not identify Codex's main-process window services factory.",
...
function formatWindowsShellHarnessFailure(
  diagnostics: WindowsShellHarnessCandidateDiagnostic[],
): string {
  const lines = [
    "Windows shell harness patch point not found.",
    "",
    "CodexDC could not identify Codex's Windows SHELL guard.",
    "This usually means Codex changed the local-environment shell selection helper or already fixed the issue in a new shape.",
    "",
    `Candidate files scanned: ${diagnostics.length}`,
  ];
  ...
  for (const candidate of diagnostics) {
    lines.push(
      ...
      `  Bytes: ${candidate.bytes}`,
      `  Marker already present: ${candidate.diagnostics.hasMarker ? "yes" : "no"}`,
      `  Restrictive SHELL guards: ${candidate.diagnostics.restrictiveMatches}`,
      `  Permissive SHELL guards: ${candidate.diagnostics.permissiveMatches}`,
      `  SHELL mentions: ${candidate.diagnostics.shellMentions}`,
      `  powershell mentions: ${candidate.diagnostics.powershellMentions}`,
    );
    if (candidate.parserError) {
      lines.push(`  Parser error: ${candidate.parserError}`);
    }
...
function macAppManagementFix(target: string, code: string | undefined): string {
  const permissionSteps =
    ...
    `Run "codexdc repair" in your terminal.\n`;
  const sudoFallback =
    code === "EACCES"
      ? `If Codex.app is root-owned and repair still cannot write to it, run "sudo codexdc repair".\n`
      : "";
...
function formatCodexRunningError(codex: CodexInstall, open: OpenReport): string {
  ...
  return (
    ...
    `CodexDC cannot safely patch app.asar while Codex is running. ` +
...
function installWindowsManagedAppLauncher(codex: CodexInstall): { shortcutPaths: string[] } | null {
  ...
  const commandPath = join(shimDir, "codexdc-codex.cmd");
  const appUserModelIdLine = codex.appUserModelId
    ? `set "CODEXDC_APP_USER_MODEL_ID=${codex.appUserModelId}"\r\n`
    : "";
  writeFileSync(
    commandPath,
  ...
  const desktopShortcut = join(homedir(), "Desktop", "CodexDC.lnk");
  if (createWindowsCodexShortcut(desktopShortcut, commandPath, iconPath, codex.appUserModelId)) {
...
function createWindowsCodexShortcut(
  shortcutPath: string,
  targetPath: string,
  iconPath?: string,
  ...
  try {
    ...
    const helperPath = join(assetsDir, "windows-shortcut.ps1");
    if (existsSync(helperPath)) {
      ...
      if (appUserModelId) args.push("-AppUserModelId", appUserModelId);
    ...
    const script = [
      ...
      `$shortcut.IconLocation = "${escapePowerShellDoubleQuotedString(iconPath ?? targetPath)},0"`,
      "$shortcut.Save()",
    ].join("; ");
    execFileSync(
      "powershell.exe",
