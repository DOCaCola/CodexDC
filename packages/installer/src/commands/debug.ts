...
function mainCodexProcesses(codex: CodexInstall, processes: ProcessInfo[]): ProcessInfo[] {
  return processes.filter((p) => matchesCodexMainExecutable(codex, p.command));
}

export function matchesCodexMainExecutable(
  codex: Pick<CodexInstall, "executable">,
  command: string,
): boolean {
  const executable = normalizePath(codex.executable);
  const normalizedCommand = normalizePath(command);
  return normalizedCommand === executable || normalizedCommand.startsWith(`${executable} `);
}
...
function hasWindowServicesMarker(asarPath: string): boolean {
  ...
  try {
    ...
    for (const file of files) {
      if (!file.split(String.fromCharCode(92)).join("/").startsWith("/.vite/build/") || !file.endsWith(".js")) continue;
...
function printDebugReport(report: DebugReport): void {
  console.log(kleur.bold("codexdc debug"));
  console.log();
  ...
  console.log();
  printDataPaths("codexdc data paths", report.codexPlusPlusPaths);
}
