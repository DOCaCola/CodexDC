...
export function createTweak(target: string, opts: CreateTweakOpts = {}): void {
  ...
  writeJson(dir, "package.json", {
    ...
    type: "commonjs",
    scripts: {
      validate: "codexdc validate-tweak .",
    },
    devDependencies: {
      "@codexdc/sdk": "^0.1.3",
    },
  ...
  console.log(kleur.green().bold("✓ Created CodexDC tweak"));
  ...
  console.log(`  2. Run ${kleur.cyan(`codexdc validate-tweak ${dir}`)}`);
  console.log("  3. Copy or symlink this directory into your CodexDC tweaks directory");
}
...
\`\`\`sh
codexdc validate-tweak .
\`\`\`

Install by copying or symlinking this directory into your CodexDC tweaks directory.
`;
}
