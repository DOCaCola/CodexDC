...
export const DEFAULT_LOCAL_SIGNING_IDENTITY = "CodexDC Local Signing";
...
export function signCodexApp(appRoot: string, opts: CodeSigningOptions = {}): CodeSigningResult | null {
  ...
  walkAndSign(join(appRoot, "Contents", "MacOS"), signingIdentity);
...
export function prepareCodeSigning(opts: CodeSigningOptions = {}): PreparedSigningIdentity | null {
  ...
  requireExecutable("security", "macOS security is required to find CodexDC's local signing identity.");
  ...
  requireExecutable("openssl", "macOS openssl is required to create CodexDC's local signing identity.");
...
function createLocalSigningIdentity(identityName: string): PreparedSigningIdentity {
  const dir = mkdtempSync(join(tmpdir(), "codexdc-signing-"));
  try {
