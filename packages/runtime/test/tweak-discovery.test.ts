...
test("discoverTweaks accepts tweaks without githubRepo", () => {
  withTempDir((root) => {
    const tweak = writeTweak(root, "local-only", {
      id: "com.example.local-only",
      name: "Local Only",
      version: "0.1.0",
      scope: "renderer",
    });
    ...
    assert.equal(discovered.length, 1);
    assert.equal(discovered[0]?.manifest.id, "com.example.local-only");
  });
});
