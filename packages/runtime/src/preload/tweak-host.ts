...
import { clearSlashCommands, registerSlashCommand } from "./slash-commands";
import type {
  CodexCdpStatus,
  CodexCdpTarget,
...
export async function startTweakHost(): Promise<void> {
  ...
  for (const t of tweaks) {
    ...
    try {
      ...
      clearSlashCommands(t.manifest.id);
      console.error("[codexdc] tweak load failed:", t.manifest.id, e);
      try {
        ipcRenderer.send(
          "codexpp:preload-log",
          "error",
  ...
  console.info(
    `[codexdc] renderer host loaded ${loaded.size} tweak(s):`,
...
export function teardownTweakHost(): void {
  for (const [id, t] of loaded) {
    try {
      ...
      console.warn("[codexdc] tweak stop failed:", id, e);
    } finally {
...
function makeRendererApi(manifest: TweakManifest, paths: UserPaths): TweakApi {
  ...
  return {
    ...
    composer: {
      slashCommands: {
        register: (command) => registerSlashCommand(id, command),
      },
    },
    react: {
    ...
    codex: rendererCodexApi(
      id,
      manifest.permissions?.includes("codex-runtime") === true,
    ),
  };
}

function rendererCodexApi(
  tweakId: string,
  canRestartAppServer: boolean,
): NonNullable<TweakApi["codex"]> {
  return {
    runtime: {
      getInfo: async () => {
      ...
      restartAppServer: async (options = {}) => {
        if (!canRestartAppServer) {
          throw new Error(`tweak ${tweakId} requires codex-runtime permission`);
        }
        await ipcRenderer.invoke(DESKTOP_MESSAGE_FROM_VIEW, {
          type: "codex-app-server-restart",
          hostId: options.hostId ?? "local",
          intent: "restart",
          killCodexProcess: options.killCodexProcess ?? false,
        });
      },
    },
    files: {
      stat: (path: string) =>
        ipcRenderer.invoke("codexpp:codex-file-stat", tweakId, path),
      showInFolder: (path: string) =>
        ipcRenderer.invoke("codexpp:codex-file-show-in-folder", tweakId, path),
    },
    windows: {
      create: (options) =>
