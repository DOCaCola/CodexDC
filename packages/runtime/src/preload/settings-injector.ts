...
const CODEXDC_RELEASES_URL = "https://github.com/DOCaCola/CodexDC/releases";
...
export function startSettingsInjector(): void {
  ...
  const obs = new MutationObserver(scheduleInjection);
...
function scheduleInjection(): void {
  if (state.injectionFrame !== null) return;
  state.injectionFrame = requestAnimationFrame(() => {
    state.injectionFrame = null;
    tryInject();
    maybeDumpDom();
  });
}
...
function syncPagesGroup(): void {
  ...
  if (!outer) return;
  if (!isSettingsSidebarCandidate(outer, true)) {
...
function renderCodexPlusPlusConfig(card: HTMLElement, config: CodexPlusPlusConfig): void {
  ...
  card.appendChild(rowSimple("Release channel", "Stable releases from DOCaCola/CodexDC"));
...
function updateChannelRow(config: CodexPlusPlusConfig): HTMLElement {
  ...
  if (config.updateChannel === "custom") {
    action?.appendChild(
      compactButton("Edit", () => {
        const repo = window.prompt("GitHub repo", config.updateRepo || "DOCaCola/CodexDC");
        if (repo === null) return;
...
function updateChannelSummary(config: CodexPlusPlusConfig): string {
  if (config.updateChannel === "custom") {
    return `${config.updateRepo || "DOCaCola/CodexDC"} ${config.updateRef || "(no ref set)"}`;
  }
  ...
  action?.appendChild(
    compactButton("Open Issue", () => {
      ...
      void ipcRenderer.invoke(
        ...
        `https://github.com/DOCaCola/CodexDC/issues/new?title=${title}&body=${body}`,
      );
    }),
  );
  return row;
...
function isSettingsSidebarCandidate(el: HTMLElement, allowKnownRoot = false): boolean {
  ...
  if (isCodexPpSettingsLabelSet(labels)) return true;

  // Settings search can temporarily filter out nearly all native nav labels.
  // Keep a previously validated, still-visible sidebar stable during that
  // transient state instead of removing and reinjecting our groups.
...
function isCodexPpInjectedSettingsGroupPlacementValid(group: HTMLElement): boolean {
  ...
  for (let depth = 0; node && depth < 4; depth++) {
    ...
    if (isSettingsSidebarCandidate(node, true)) return true;
