...
const LAUNCHD_LABEL = "com.codexdc.watcher";
const WATCHER_LOG = join(homedir(), "Library", "Logs", "codexdc-watcher.log");
...
export function getWatcherHealth(userRoot: string): WatcherHealth {
  ...
  checks.push({
    ...
    detail: state ? `CodexDC ${state.version ?? "(unknown version)"}` : "state.json is missing",
  });
  ...
  checks.push({
    ...
    detail: autoUpdate ? "enabled" : "disabled in CodexDC config",
  });
...
function selfUpdateCheck(state: SelfUpdateState): WatcherHealthCheck {
  ...
  if (state.status === "failed") {
    return {
      name: "last CodexDC update",
      status: "warn",
      detail: state.error ? `failed ${at}: ${state.error}` : `failed ${at}`,
    };
  }
  if (state.status === "disabled") {
    return { name: "last CodexDC update", status: "warn", detail: `skipped ${at}: automatic refresh disabled` };
  }
  if (state.status === "updated") {
    return { name: "last CodexDC update", status: "ok", detail: `updated ${at} to ${state.latestVersion ?? "new release"}` };
  }
  if (state.status === "up-to-date") {
    return { name: "last CodexDC update", status: "ok", detail: `up to date ${at}` };
  }
  return { name: "last CodexDC update", status: "warn", detail: `checking since ${at}` };
}
...
function checkLaunchdWatcher(appRoot: string): WatcherHealthCheck[] {
  ...
  if (plist) {
    ...
    checks.push({
      ...
      status: plist.includes("CODEXDC_WATCHER=1") && plist.includes(" update --watcher --quiet")
        ? "ok"
        : "error",
...
function checkSystemdWatcher(appRoot: string): WatcherHealthCheck[] {
  ...
  const service = join(dir, "codexdc-watcher.service");
  const timer = join(dir, "codexdc-watcher.timer");
  const pathUnit = join(dir, "codexdc-watcher.path");
  ...
  return [
    ...
    },
    {
      ...
      status: commandSucceeds("systemctl", ["--user", "is-active", "--quiet", "codexdc-watcher.path"]) ? "ok" : "warn",
      detail: "systemctl --user is-active codexdc-watcher.path",
    },
    {
      name: "timer active",
      status: commandSucceeds("systemctl", ["--user", "is-active", "--quiet", "codexdc-watcher.timer"]) ? "ok" : "warn",
      detail: "systemctl --user is-active codexdc-watcher.timer",
    },
  ];
}
...
function checkScheduledTaskWatcher(): WatcherHealthCheck[] {
  return [
    {
      name: "logon task",
      status: commandSucceeds("schtasks.exe", ["/Query", "/TN", "codexdc-watcher"]) ? "ok" : "error",
      detail: "codexdc-watcher",
    },
    {
      name: "hourly task",
      status: commandSucceeds("schtasks.exe", ["/Query", "/TN", "codexdc-watcher-hourly"]) ? "ok" : "warn",
      detail: "codexdc-watcher-hourly",
    },
  ];
}
...
export function analyzeWatcherLogTail(tail: string): WatcherHealthCheck {
  const hasError = /✗ codexdc failed|codexdc failed|error|failed/i.test(tail);
  const needsManualRepair =
    hasError &&
    /Cannot write to .*Codex.*\.app|App Management|file ownership|sudo codexdc (?:install|repair)|EACCES|EPERM/i.test(tail);
  return {
    name: "watcher log",
    status: hasError ? "warn" : "ok",
    detail: hasError
      ? needsManualRepair
        ? "auto-repair needs app permissions; run `codexdc repair` from Terminal"
...
function summarize(watcher: string, checks: WatcherHealthCheck[]): WatcherHealth {
  ...
  const summary =
    status === "ok"
      ? "CodexDC should automatically repair itself after Codex updates."
