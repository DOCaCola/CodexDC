...
function installBrowserUiIpcHandlers(log: LogFn): void {
  ...
  process.once("exit", () => {
    for (const pending of bridgeRequests.values()) {
      clearTimeout(pending.timer);
      pending.reject(new Error("CodexDC browser UI server stopped"));
    }
...
(() => {
  ...
  async function bridge(method, args = []) {
    ...
    if (!body.ok) throw new Error(body.error || "CodexDC browser bridge failed");
    return body.value;
  }
...
function requireOptions(): BrowserUiServerOptions {
  if (!activeOptions) throw new Error("CodexDC browser UI server is not configured");
