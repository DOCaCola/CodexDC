...
export const DEFAULT_TWEAK_STORE_INDEX_URL =
  "https://github.com/DOCaCola/CodexDC-Tweaks/releases/download/catalog/index.json";
export const TWEAK_STORE_REVIEW_ISSUE_URL =
  "https://github.com/DOCaCola/CodexDC/issues/new";
...
export function normalizeStoreEntry(input: unknown): TweakStoreEntry {
  ...
  if (!manifest.githubRepo) {
    throw new Error(`Store entry ${manifest.id} is missing manifest githubRepo`);
  }
  ...
  return {
    ...
    manifest,
    repo,
    path: normalizeTweakPath(entry.path),
