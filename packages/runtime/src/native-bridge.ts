...
export class NativeBridge {
  ...
  loadModule(ctx: NativeTweakContext, options: NativeModuleLoadOptions): NativeModuleRef {
    ...
    if (kind !== "node-addon") {
      throw new Error(
        `${kind} native modules must be loaded through a .node Objective-C++ shim in CodexDC 1.0.0`,
      );
    }
  ...
  launchHelper(ctx: NativeTweakContext, options: NativeHelperLaunchOptions): NativeHelperRef {
    ...
    if ((options.transport ?? "stdio") !== "stdio") {
      throw new Error("native helpers support only stdio transport in CodexDC 1.0.0");
    }
    if ((options.restart ?? "never") !== "never") {
      throw new Error("native helper restart policies are not available in CodexDC 1.0.0");
    }
  ...
  private async createNativeInstance(
    ...
    if (typeof fn !== "function") {
      const label = moduleId ? `native module ${ctx.id}:${moduleId}` : "CodexDC native host";
  ...
  private loadNativeHost(required: boolean): unknown | null {
    ...
    if (!nativeHostPath || !existsSync(nativeHostPath)) {
      const error = new Error("CodexDC native host is not installed");
    ...
    try {
      ...
      this.log("info", "loaded CodexDC native host", { path: nativeHostPath });
      return this.nativeHostExports;
    } catch (error) {
      this.nativeHostLoadError = error instanceof Error ? error : new Error(String(error));
      this.log("error", "failed to load CodexDC native host", this.nativeHostLoadError);
  ...
  private readNativeHostCapabilities(host: unknown): Record<string, unknown> {
    ...
    try {
      ...
      this.log("warn", "CodexDC native host capability probe failed", error);
      return {};
    }
  }
