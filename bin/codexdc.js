...
if (run.error) {
  console.error(`[!] Could not start codexdc: ${run.error.message}`);
  process.exit(1);
}
process.exit(run.status ?? 1);

function bootstrap() {
  console.error("[codexdc] First run from source install; building CLI...");
  runOrExit(npmCommand(), installArgs(), "install codexdc dependencies");
  runOrExit(npmCommand(), ["run", "build"], "build codexdc");
}
